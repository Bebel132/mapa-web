...
const preposicoesEstados = {
    "Acre": "do", "Alagoas": "de", "Amapá": "do", "Amazonas": "do",
    "Bahia": "da", "Ceará": "do", "Distrito Federal": "do", "Espírito Santo": "do",
    "Goiás": "de", "Maranhão": "do", "Mato Grosso": "do", "Mato Grosso do Sul": "do",
    "Minas Gerais": "de", "Pará": "do", "Paraíba": "da", "Paraná": "do",
    "Pernambuco": "de", "Piauí": "do", "Rio de Janeiro": "do", "Rio Grande do Norte": "do",
    "Rio Grande do Sul": "do", "Rondônia": "de", "Roraima": "de",
    "Santa Catarina": "de", "São Paulo": "de", "Sergipe": "de", "Tocantins": "do"
};
...
function preencherPopUp(dadosEstado) {
    const porcentagemLimpa = parseFloat(dadosEstado.porcentagem.replace('%', '').replace(',', '.'));
    const pessoasComInternet = parseInt((porcentagemLimpa * dadosEstado.pessoas) / 100);

    const preposicao = preposicoesEstados[dadosEstado.estado] || "de";

    document.getElementById("pop-estado").textContent = `Dados ${preposicao} ${dadosEstado.estado}`;
    document.getElementById("pop-area").textContent = dadosEstado.area.toLocaleString('pt-br') + " km²";
    document.getElementById("pop-populacao").textContent = dadosEstado.pessoas.toLocaleString('pt-br');
    
    document.getElementById("pop-barra").style.width = porcentagemLimpa + "%";
    document.getElementById("pop-porcentagem-texto").textContent = dadosEstado.porcentagem;
    document.getElementById("pop-equivalencia").textContent = `Isto é ${pessoasComInternet.toLocaleString('pt-br')} pessoas com acesso à internet`;
    ...
    mapaDados.forEach(async d => {
        ...
        g.addEventListener("mouseenter", () => {
            g.querySelectorAll("path").forEach(path => {
                if (path.classList !== 'ativo') path.classList.add("hover");
            });
        });
        ...
        g.addEventListener("click", () => {
            ...
            document.querySelectorAll(".ativo").forEach(a => a.classList.remove("ativo"));
